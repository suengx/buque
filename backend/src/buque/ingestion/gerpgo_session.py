...
class GerpgGoSession:
    ...
    def export_inventory_product(self, monitor_date: date) -> Path:
        ...
        gerpgo_ui.export_inventory_product(self.page, target)
        return target
    ...
    def export_inventory_multi_platform(self, monitor_date: date) -> list[Path]:
        ...
        if INVENTORY_MULTI_PLATFORM_TABS:
            for tab_name in INVENTORY_MULTI_PLATFORM_TABS:
                ...
                gerpgo_ui.export_inventory_multi_platform(self.page, target, tab_name=tab_name)
                files.append(target)
        else:
            target = self._timestamp_name("inventory_multi_platform", monitor_date)
            gerpgo_ui.export_inventory_multi_platform(self.page, target)
            files.append(target)
        return files

    def export_orders(self, monitor_date: date) -> Path:
        target = self._timestamp_name("orders", monitor_date)
        gerpgo_ui.export_orders(self.page, target)
        return target
