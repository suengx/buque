...
**监控助手**:
计划人员与系统多轮对话、追问风险证据与建议的界面；补雀唯一的用户侧 LLM 入口。
_Avoid_: Agent 深度分析（独立按钮）, 多套对话入口

**快照会话**:
监控助手的一次对话上下文，必须绑定某次监控快照（snapshot）；从 SKU 分析卡进入时可预填 Basic SKU 与仓库焦点。
_Avoid_: 无快照的泛聊, 跨快照混用数据

**解释草稿**:
监控助手通过专用工具提出的结构化解释与建议，采纳前不写入正式解释记录。
_Avoid_: 把 Chat 自由文本直接当正式解释, 助手自动落库

**采纳解释**:
人工确认解释草稿并写入 `fact_agent_explain` 的显式动作；可覆盖该事件上一次的展示解释。
_Avoid_: 自动采纳, 与提交反馈混为一键

**提交反馈**:
人工对建议动作的采纳、驳回或部分采纳结论，写入反馈留痕；与采纳解释分步完成。
_Avoid_: 用反馈表存解释正文

**规则解释**:
日批流水线由解释规则引擎确定性生成的解释，是 SKU 卡与清单的默认展示；无 Chat 交互时仍完整可用。
_Avoid_: 日批调用 LLM, 把规则解释称为 Agent 解释
...
- **监控助手** 会话绑定一个快照；可选焦点为 **Basic SKU** + **WAREHOUSE** 仓库
- 日批产出 **规则解释** 写入正式解释记录；**监控助手** 产出 **解释草稿**，经 **采纳解释** 后才更新同一记录上的展示内容
- **采纳解释** 与 **提交反馈** 是两次人工动作：前者沉淀解释与证据，后者闭环建议是否采纳
...
> **Dev:** 「专员在监控助手里改了解释，和早上日批的规则解释不一致，以哪个为准？」
> **Domain expert:** 「清单默认还是规则解释。她在对话里若认可助手草稿，点采纳解释才更新正式记录；建议动作是否采纳另走提交反馈。灯色和触发规则不因 Chat 改变。」
...
| # | 议题 | 决议 |
|---|---|---|
| 9 | 用户侧 Agent 形态 | **监控助手 Chat** 为唯一 LLM 入口；Claude Agent SDK |
| 10 | 与日批 LLM 边界 | 日批仅 **规则解释**；LLM 仅用户发起 Chat |
| 11 | 解释采纳机制 | **解释草稿** → **采纳解释** → `fact_agent_explain`；**提交反馈** 分步 |
| 12 | 会话上下文 | **快照会话** 绑 `snapshot_id`；可选预填 sku / warehouse |
| 13 | 身份与审计 | 快速登录注册后 **`user_id`** 贯穿会话与采纳；Chat 与 auth 并行开发 |

架构取舍详见 [`docs/adr/`](docs/adr/)（含 [0003-chat-agent-claude-sdk](docs/adr/0003-chat-agent-claude-sdk.md)）。
